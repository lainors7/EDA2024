...
ficheros_requeridos=("CityInfo.h" "CityInfo.cc" "Coordinates.h" "Coordinates.cc" "City.h" "City.cc" "Map.h" "Map.cc" "maptest.cc")
clases_compiladas="CityInfo.o Coordinates.o City.o Map.o"
aplicacion=maptest
directorio=pruebas
compilador=g++
valgrind="valgrind -q"
...
function ejecutar_pruebas() {
    ...
    if [[ $numero_pruebas -eq 0 ]]; then
        ...
        if [[ $pruebas_pasadas -eq $numero_pruebas ]]; then
          echo "Funcionan todas las pruebas, recuerda hacer tus propias pruebas"
        else
          echo "Funcionan $pruebas_pasadas pruebas de $numero_pruebas"
        fi
    fi
}
